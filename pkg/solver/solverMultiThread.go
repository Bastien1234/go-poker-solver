...
func SolveMultiThread() {
	...
	*/
	...
	solvedHands := solvedHandsStruct{}
	solvedHands.solvedHands = make(map[string]int)
	...
	for iter1 := 0; iter1 < constants.Iterations1; iter1++ {
		...
		*/
		...
		if duplicatesOOP1 == false && duplicatesOOP2 == false && duplicatesOOP3 == false && duplicatesOOP4 == false && duplicatesIP1 == false && duplicatesIP2 == false && duplicatesIP3 == false && duplicatesIP4 == false {
			...
			if val, ok := solvedHands.solvedHands[stringHandOOP]; ok {
				OOPValue = val
			} else {
				solvedHands.mu.Lock()
				defer solvedHands.mu.Unlock()
				OOPValue = handSolver.HandSolver(boardOOP)
				solvedHands.solvedHands[stringHandOOP] = OOPValue
			}

			if val, ok := solvedHands.solvedHands[stringHandIP]; ok {
				IPValue = val
			} else {
				solvedHands.mu.Lock()
				defer solvedHands.mu.Unlock()
				IPValue = handSolver.HandSolver(boardIP)
				solvedHands.solvedHands[stringHandIP] = IPValue
			}
